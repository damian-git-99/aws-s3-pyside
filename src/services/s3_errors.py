...
class S3AccessDeniedError(S3Error):
    ...
    def __init__(self, bucket_name: str):
        ...
        super().__init__(
            f"Access denied to bucket '{bucket_name}'. Check your AWS permissions."
        )
...
class S3ConnectionError(S3Error):
    ...
    def __init__(self):
        super().__init__(
            "Cannot connect to AWS. Please check your network configuration."
        )
...
class S3ObjectNotFoundError(S3Error):
    ...
    def __init__(self, key: str):
        self.key = key
        super().__init__(
            f"Object '{key}' not found in bucket. It may have been deleted by another process."
        )
...
class S3PresignedUrlError(S3Error):
    ...
    def __init__(self, key: str, reason: str):
        self.key = key
        self.reason = reason
        super().__init__(f"Failed to generate link for '{key}': {reason}")
